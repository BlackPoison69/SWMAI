...
if (isset($_GET['sucesso']) && $_GET['sucesso'] === 'sim') {
    ...
    $umid_solo_recebido = filter_input(INPUT_GET, 'UmidSolo', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    $chuva_recebida = filter_input(INPUT_GET, 'Chuva', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    ...
    try {
        ...
        // O número de colunas na instrução SQL agora corresponde ao número de valores.
        $stmt_insert = $conn->prepare("
            INSERT INTO Registro (ID_Sensor, MomentoCaptura, UmidAr, TempAr, UmidSolo, Chuva, Luminosidade, fk_Horta)
            VALUES (?, NOW(), ?, ?, ?, 0, 0, ?)
        ");
        $stmt_insert->execute([
            $id_sensor_recebido,
        ...
        echo "SUCESSO: Conexão e inserção no banco de dados bem-sucedidas. Registro de teste criado com dados de temperatura e umidade do ar e do solo.";
        exit();
    } catch (Exception $e) {
